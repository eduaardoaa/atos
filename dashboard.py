...
def grafico_linhas_por_filial(mes_referencia, filial_selecionada):
    ...
    # Normalizar nome do mês (remover acentos e converter para título)
    ...
    # Tentar encontrar o mês no mapa
    mes_num = (
        meses_map.get(mes_nome) or 
        meses_map.get(mes_nome_normalizado) or 
        meses_map.get(mes_nome[:3])  # Para pegar abreviações
    )
